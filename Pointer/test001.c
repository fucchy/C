...
int main(int argc,char *argv[]){
	...
	//
	// [新版]明解C言語 実践編 柴田望洋著(初版, ISBN4-7973-2995-5) 58ページから抜粋
	// 「ポインタ変数ptrに対する++*ptrと(*ptr)++とは同じですか？」
	// 同じです。ポインタでない変数xに対する++xが(x)++と等しいのと同じことです。
	// ただし、(*ptr)++の( )は省略できないことに注意しましょう。
	// *と++は同一の優先順位をもつ右結合の演算子だからです。
	//
	(*p)++; // ポインタpの参照先に1を加える
